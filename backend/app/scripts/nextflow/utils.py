...
def directory_is_ready(log_location, history_location, existing_id=None):
    ...
    log_path = Path(log_location, ".nextflow.log")
...
def get_process_status_from_log(log, id):
    ...
    file, given its unique ID. If it's still running, 'RUNNING' is returned.
    ...
    return ("FAILED" if match[2] != "0" else match[1]) if match else "RUNNING"
...
def get_process_directory(execution, id):
    ...
    options = os.listdir(os.path.join(execution.log_location, first))
    second = [option for option in options if option.startswith(second)]
    if not second: return
    return os.path.join(execution.log_location, first, second[0])